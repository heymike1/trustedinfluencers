<div>
    <div class="flex flex-wrap items-end justify-between gap-4 mb-4">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-ink-950">Creators</h1>
            <p class="mt-1 text-sm text-ink-500 tnum">{{ number_format($total) }} creators · {{ number_format($verifiedTotal) }} with verified metrics. Columns change with the platform, because what matters for a campaign differs on each.</p>
        </div>
        <a href="{{ route('creators.create') }}" class="btn-secondary">Add a creator</a>
    </div>

    {{-- Platform tabs --}}
    <div class="flex gap-0 border-b border-ink-200 mb-4 text-sm overflow-x-auto">
        <button type="button" wire:click="$set('platform', '')" class="px-3.5 py-2.5 whitespace-nowrap {{ $platform === '' ? 'font-semibold text-ink-950 border-b-2 border-brand-600 -mb-px' : 'font-medium text-ink-500 hover:text-ink-900' }}">All platforms</button>
        @foreach($platforms as $p)
            <button type="button" wire:click="$set('platform', '{{ $p->value }}')" class="inline-flex items-center gap-1.5 px-3.5 py-2.5 whitespace-nowrap {{ $platform === $p->value ? 'font-semibold text-ink-950 border-b-2 border-brand-600 -mb-px' : 'font-medium text-ink-500 hover:text-ink-900' }}">
                <x-platform-icon :platform="$p" class="size-3.5" :colored="true" /> {{ $p->label() }} <span class="font-normal text-ink-400 tnum">{{ $platformCounts[$p->value] ?? 0 }}</span>
            </button>
        @endforeach
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 mb-3" x-data="{ more: {{ $followersMin !== '' || $followersMax !== '' || $medianViewsMin !== '' || $averageViewsMin !== '' || $engagementMin !== '' ? 'true' : 'false' }} }">
        <div class="relative">
            <svg class="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 text-ink-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="M20 20l-3.5-3.5"/></svg>
            <input type="search" wire:model.live.debounce.300ms="search" placeholder="Name, @handle or category" class="input w-60 pl-8 py-1.5">
        </div>
        <select wire:model.live="category" class="input w-auto py-1.5">
            <option value="">Any category</option>
            @foreach($categories as $c)
                <option value="{{ $c->slug }}">{{ $c->name }}</option>
            @endforeach
        </select>
        <select wire:model.live="claimed" class="input w-auto py-1.5">
        ...
        <button type="button" @click="more = !more" class="chip" :class="more && 'border-brand-600 text-brand-700'">Numbers <svg class="size-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M6 9l6 6 6-6"/></svg></button>
        ...
        <div class="ml-auto flex items-center gap-2 text-xs text-ink-500">
            Sort
            <select wire:model.live="sort" class="input w-auto py-1.5 text-xs">
            ...
            <div class="flex rounded-md border border-ink-200 bg-white p-0.5">
                <button type="button" wire:click="$set('view', 'table')" class="rounded px-2 py-1 {{ $view === 'table' ? 'bg-brand-600 text-white' : 'text-ink-600' }}">Table</button>
                <button type="button" wire:click="$set('view', 'cards')" class="rounded px-2 py-1 {{ $view === 'cards' ? 'bg-brand-600 text-white' : 'text-ink-600' }}">Cards</button>
            </div>
        </div>

        <div x-show="more" x-cloak class="w-full grid gap-3 rounded-md border border-ink-200 bg-white p-3 sm:grid-cols-5">
            <div>
