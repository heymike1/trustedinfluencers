...
<svg viewBox="0 0 {{ $w }} {{ $h }}" width="100%" height="{{ $h }}" class="block" role="img" {{ $attributes }}>
    ...
    <path d="{{ $area }}" fill="#1d6ef5" fill-opacity="0.10"></path>
    <path d="{{ $path }}" fill="none" stroke="#1d6ef5" stroke-width="2" stroke-linejoin="round"></path>
    @foreach($markers as [$index, $text])
        ...
        <circle cx="{{ round($x($index), 1) }}" cy="{{ round($y($points[$index]), 1) }}" r="3" fill="#1d6ef5"></circle>
